...
import com.andreykliev.input_output.read.ReadingFromFile;
import com.andreykliev.input_output.write.WritngToConsole;
import com.andreykliev.input_output.write.WritingToFile;
...
class CommandLineParser {
    ...
    void parse(String[] args) {
        ...
        if (args.length >= 3) {
            ...
            if (DEBUG) {
                Main.writer = new WritngToConsole();
                argsCounter++;
            } else {
                Main.writer = new WritingToFile(new File(args[argsCounter++]));
            }
            ...
            if (inputFileNames.size() < 1) {
                ...
                for (String fileName : inputFileNames) {
                    Main.readers.add(new ReadingFromFile(new File(fileName)));
                }
            }
        } else {
